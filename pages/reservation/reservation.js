...
Page({
  ...
  loadData(id){
    ...
    network.requestLoading(url,params,"加载中...",function(res){
      that.setData({
        ...
        actualAmount:res.result.earnestMoney>0?res.result.earnestMoney:res.result.totalAmount
      });
      that.getMoney();
    });
  },
  ...
  changePayMode(){
    ...
    var actualAmount=payMode==0?this.data.detail.earnestMoney:this.data.detail.totalAmount;
    this.setData({
      payMode:payMode,
  ...
  getMoney(){
    ...
    var car=this.data.detail;
    var discountFee=this.data.discountFee;
    totalMoney+=car.totalAmount;//租车费
    totalMoney+=car.insuranceFee*day;//保险费
    totalMoney+=car.basicServiceFee*day;//基础服务费
    totalMoney+=car.otherFee*day;//其它费用
    totalMoney-=discountFee;//折扣费用
    this.setData({
  ...
  tobuy(event){
    ...
    var ops = {
      ...
      //"basicServiceFee": that.data.detail.basicServiceFee,//基础服务费
      //"otherFee": that.data.detail.otherFee,//其它费用
      "totalAmount": that.data.totalAmount,//总费用

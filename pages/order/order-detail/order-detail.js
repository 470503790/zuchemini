...
Page(Object.assign({}, Zan.NoticeBar, Zan.Dialog, {
    ...
    loadData: function (success) {
        ...
        network.requestLoading(url, {
            ...
            var km = detail.kmNum - 60;
            var hour = detail.hourNum - 8;
            if (km > 0) {
                totalBeyond10KmFee = km / 10 * detail.beyond10KmFee;
            }
            if (hour > 0) {
                totalBeyondHourFee = hour * detail.beyondHourFee;
            }
            that.setData({
                orderDetail: detail,
                ...
                addresses: JSON.parse(detail.address)
            });
            //判断为 待退款
            if (res.result.status == "待退款") {
                that.setData({
                    movable1: {
                        text: "我们将在24小时内为您办理退款业务，款项将原路退回。"
                    }
                })
            }
    ...
    setAddress: function (e) {
        ...
        for (var i = 0; i < addresses.length; i++) {
            if (addresses[i].title == title) {
                ...
                wx.openLocation({
                    latitude: addresses[i].location.latitude,
                    longitude: addresses[i].location.longitude,
                    name: addresses[i].location.name,
                    address: addresses[i].location.address,
                    scale: 28
                })
                return;
            }
